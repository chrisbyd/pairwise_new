...
test_log_dir = './log/test_log/'
test_log_path = test_log_dir + args.resume +'reverse'+ '.txt'
...
if dataset == 'sysu':
    # testing set
    query_img, query_label, query_cam = process_query_sysu_reverse(data_path, mode=args.mode)
    gall_img, gall_label, gall_cam = process_gallery_sysu_reverse(data_path, mode=args.mode)
...
if dataset == 'regdb':
    ...
    print(
        'POOL5:   Rank-1: {:.2%} |Rank-2: {:.2%} |Rank-3: {:.2%} |Rank-4: {:.2%} |Rank-5: {:.2%} |Rank-6: {:.2%} |Rank-7: {:.2%} |Rank-8: {:.2%} | Rank-9: {:.2%} | Rank-10: {:.2%}| mAP: {:.2%}'.format(
            cmc[0], cmc[1], cmc[2], cmc[3], cmc[4], cmc[5], cmc[6], cmc[7], cmc[8], cmc[9], mAP_pool),
        file=test_log_file)
    ...
    for trial in range(10):
        gall_img, gall_label, gall_cam = process_gallery_sysu_reverse(data_path, mode=args.mode, trial=trial)
